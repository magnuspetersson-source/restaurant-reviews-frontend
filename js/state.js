...
(function () {
  ...
  function setReviews(reviews) {
    ...
    for (const r of state.data.reviews) {
      ...
      if (Number.isFinite(id)) byId[id] = r;
    }
  ...
  function selectReview(id) {
    const n = (id === null || id === undefined || id === "") ? null : Number(id);
    state.ui.selectedReviewId = Number.isFinite(n) ? n : null;

    // Router is optional
    if (window.RR_ROUTER && typeof window.RR_ROUTER.setSelectedId === "function") {
      window.RR_ROUTER.setSelectedId(state.ui.selectedReviewId);
    }
    emit();
  }
